...
test_batch_diff: bin src/main.cpp
	@pis=`echo $(PIs1)`; \
	thetas=`echo $(THETAs1)`; \
	length=`echo $$pis | wc -w`; \
	for i in `seq 1 $$length`; do \
		pi=`echo $$pis | cut -d ' ' -f $$i`; \
		theta=`echo $$thetas | cut -d ' ' -f $$i`; \
		python gen.py -p $$pi -t $$theta -s $(SIMD) -u 1 $(OPTIONS); \
		g++ src/main.cpp -o bin/test $(TEST_FLAGS) -DTEST; \
		./bin/test 128; \
	done; \
	pis=`echo $(PIs10)`; \
	thetas=`echo $(THETAs10)`; \
	length=`echo $$pis | wc -w`; \
	for i in `seq 1 $$length`; do \
		pi=`echo $$pis | cut -d ' ' -f $$i`; \
		theta=`echo $$thetas | cut -d ' ' -f $$i`; \
		python gen.py -p $$pi -t $$theta -s $(SIMD) -u 10 $(OPTIONS); \
		g++ src/main.cpp -o bin/test $(TEST_FLAGS) -DTEST; \
		./bin/test 128; \
	done
...
parameters: 
	...
	for i in `seq 1 $$length`; do \
		pi=`echo $$pis | cut -d ' ' -f $$i`; \
		theta=`echo $$thetas | cut -d ' ' -f $$i`; \
		python gen.py -p $$pi -t $$theta -s $(SIMD) -u $(UNROLL) $(OPTIONS) --print; \
	done
