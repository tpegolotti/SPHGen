...
int main(int argc, char **argv)
{
    ...
    unsigned char test_m[STRING_LEN];
    ...
    while(multiplier > 1){
        ...
        for(int i = 0; i < num_runs; i++){
            EVP_MAC_init(mctx, composite_key, sizeof(composite_key), NULL);
            EVP_MAC_update(mctx, test_m, sizeof(test_m));
            EVP_MAC_final(mctx, out, &out_len, sizeof(out));
        }
        end = stop_tsc(start);
